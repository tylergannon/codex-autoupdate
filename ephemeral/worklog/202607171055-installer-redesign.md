...
decision: Uninstall retains logs and cache for diagnosis and says so explicitly; only the LaunchAgent plist and canonical executable are removed.
review: Claude Fable implementation round 04 independently reran the gates and traced installer semantics, concluding only nitpicks remain; use the real post-tag install to resolve launchctl kickstart behavior that a fake runner cannot prove.
